...
import Driver.HelperClasses.UserManualHelper;
...
public class FileChooser {
    ...
    /**
    * Creates a user manual dialog that displays a tree structure of chapters and subsections
    * on the left panel and shows the corresponding user manual text on the right panel.
    * User manual text is updated based on the selected tree node.
    */
    public static void createUserManualDialog() {
        // Create the user manual dialog
        JDialog userManualDialog = new JDialog();
        userManualDialog.setTitle("Használati útmutató");
        userManualDialog.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        userManualDialog.setSize(1100, 700);

        // Create the left panel for chapters and subsections
        ...
        // Create the right panel for user manual text
        ...
        // Create a split pane for left and right panels
        JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, leftScrollPane, rightScrollPane);
        splitPane.setDividerLocation(350);

        // Add the split pane to the user manual dialog
        userManualDialog.getContentPane().add(splitPane);

        // Create the tree with the chapter data
        JTree tree = new JTree(UserManualHelper.createChapterData());
        ...
        tree.addTreeSelectionListener(e -> {
            ...
            if (selectedNode != null && selectedNode.isLeaf()) {
                ...
                if (parent != null) {
                    ...
                    if (grandparent != null) {
                        String chapter = grandparent.toString();
                        String subsection = parent.toString();
                        String subsubsection = selectedNode.toString();
                        String text = UserManualHelper.getTextForChapter(subsubsection);
                        displayUserManualText(text, rightPanel);
                    }
                }
            }
        });

        // Add the tree to the left panel
    ...
    /**
     * Displays the user manual text in the right panel.
     *
     * @param text        the user manual text to display
     * @param rightPanel  the panel where the text will be displayed
     */
    private static void displayUserManualText(String text, JPanel rightPanel) {
        ...
        // Replace the delimiter with line breaks
        ...
        Font textFont = new Font(Font.SANS_SERIF, Font.PLAIN, 13);
