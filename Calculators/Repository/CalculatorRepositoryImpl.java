...
public class CalculatorRepositoryImpl implements CalculatorRepository {
    ...
    */
    ...
    //    setCalculated(index, true);
    }

 /*   @Override
    public boolean isCalculated(int index) {
        return fileToResultIndexMap.containsKey(index);
    }

    @Override
    public void setCalculated(int index, boolean calculated) {
        if (calculated) {
            fileToResultIndexMap.put(index, fileToResultIndexMap.getOrDefault(index, noiseEmissionResults.size()));
        } else {
